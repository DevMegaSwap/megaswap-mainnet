...
contract UniswapV2Router02 {
    ...
    function getReserves(address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IUniswapV2PairRouter(pairFor(tokenA, tokenB)).getReserves();
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = IUniswapV2PairRouter(pair).burn(to);
        (address token0,) = sortTokens(tokenA, tokenB);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            IUniswapV2PairRouter(pairFor(input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
